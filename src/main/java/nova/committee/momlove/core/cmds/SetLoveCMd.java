    ...
    public static int execute(CommandContext<CommandSourceStack> context, Collection<ServerPlayer> players) {
        for (Player player : players) {
            try {
                ...
                context.getSource().sendSuccess(new TranslatableComponent(notIn ? "momlove.love.success" : "momlove.love.duplicate", player.getName().getString()), true);
            } catch (Exception e) {
                e.printStackTrace();
                context.getSource().sendFailure(new TranslatableComponent("momlove.love.failure", player.getName().getString()));
            }
        }
