...
public class MomLove {
    ...
    public static boolean setLove(EntityPlayer player, boolean byKey) {
        final boolean b = MomLove.config.getUuidData().add(player.getUniqueID());
        ConfigHandler.onChange();
        if (b) LOGGER.info("Set love for player {}", player.getDisplayName());
        if (b)
            player.addChatComponentMessage(new ChatComponentTranslation(byKey ? "msg.momlove.keys.success" : "msg.momlove.love.success", new ChatComponentTranslation("momlove.appellation.you.lower").getFormattedText()));
        return b;
    }

    public static boolean unLove(EntityPlayer player) {
        final boolean b = MomLove.getConfig().getUuidData().remove(player.getUniqueID());
        ConfigHandler.onChange();
        if (b) LOGGER.info("Unlove player {}", player.getDisplayName());
        if (b)
            player.addChatComponentMessage(new ChatComponentTranslation("msg.momlove.unlove.success", new ChatComponentTranslation("momlove.appellation.you.upper").getFormattedText()));
        return b;
    }
